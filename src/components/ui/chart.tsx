...
function ChartTooltipContent({
  ...
  return (
    <div
      className={cn(
        'grid min-w-[8rem] items-start gap-1.5 rounded-md border border-line-hairline bg-surface-card px-2.5 py-1.5 text-xs shadow-overlay',
        className,
      )}
    >
              ...
              {formatter && item?.value !== undefined && item.name ? (
                ...
                <>
                  ...
                  >
                    ...
                    {item.value != null && (
                      <span className="font-mono font-medium tabular-nums text-ink-primary">
    ...
    >
      {payload.map((item) => {
        ...
        return (
          <div
            key={item.value}
            className={cn('flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-ink-muted')}
          >
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: unknown, key: string) {
  ...
  return config[key in config ? key : configLabelKey];
}
