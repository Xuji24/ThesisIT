...
export function getReportSectionIcon(headingText: string) {
...
export function TabIcon({ id, className = 'w-4 h-4' }: { id: string; className?: string }) {
  const Icon = TAB_ICONS[id as keyof typeof TAB_ICONS];
  if (!Icon) return null;
  return <Icon className={`${iconClass} ${className}`} aria-hidden />;
}

export function FileIcon({ className = 'w-10 h-10' }) {
  return <FileText className={`${iconClass} ${className} text-ink-muted`} aria-hidden />;
}
